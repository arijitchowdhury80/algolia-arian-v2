...
# all-MiniLM-L6-v2 output dimension -- see server/pipeline/embeddings.py.
...
class User(Base):
    ...
    read by `core.auth.acl.can_user_see()` while off [C3]. See
...
class AuditShare(Base):
    ...
    # endpoint (server/api/routers/audits.py::share_audit), not just
    # by convention.
...
class ReportChunk(Base):
    ...
    sliding window -- see server/pipeline/chunking.py. Retrieval is
    cosine similarity via pgvector's `<=>` operator, gated by
    server.pipeline.retrieval.SIMILARITY_THRESHOLD.
    """
