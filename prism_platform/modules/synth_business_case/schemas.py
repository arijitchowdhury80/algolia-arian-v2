"""Synth Business Case schemas -- input/output contracts for ROI business case synthesis."""
...
class BusinessCaseInput(BaseModel):
    """Input for the business case module -- just the domain to analyze."""
...
class ValueLever(BaseModel):
    ...
    conservative_estimate: float | None = Field(
        default=None,
        description=(
            "Annual USD impact using conservative assumptions. Float, not formatted string."
        ),
    )
