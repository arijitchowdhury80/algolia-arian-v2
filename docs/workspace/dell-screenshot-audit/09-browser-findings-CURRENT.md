...
- Results: **46 results** (re-verified 2026-07-02; originally reported 49 — the 3-result delta is normal live-catalog drift over 2 days, not a measurement error), NO price filter auto-applied in the GRID. **The "under 1000" constraint was IGNORED by the grid** — results include laptops well above $1,000. The catalog/facet engine tokenized the sentence and matched loosely; it did not understand "under 1000" as a price ceiling or "travel" as a portability signal. **Grid-level NLP = FAIL** (unchanged) — but per the correction above, the same query submitted through this box also auto-launches the Assistant, which does parse this correctly; this query specifically was not re-tested through the Assistant path but is expected to behave like the RTX-4070 case given the identical auto-launch mechanism.
...
### Key Gaps Found
1. **Catalog/facet engine = NLP FAIL, but a disconnected Assistant system already fixes it live (revised wedge — see correction in Step 2m).** The classic results GRID does not parse intent: "laptop with good battery life for travel under 1000" returns results well above $1,000 (price ceiling ignored); "best laptop for video editing" dumps 400+ undifferentiated SKUs; "gaming pc with rtx 4070 and 32gb ram" surfaces the wrong GPU tier on the grid's own top cards. The `pilot.search.dell.com/queryunderstandingapi` layer is keyword/token matching, not vector/semantic — confirmed. **BUT** every one of these same searches, submitted through the exact same header box, auto-launches a Virtual Assistant panel that correctly parses the multi-attribute intent (verified live: RTX 4070 + 32GB + budget correctly resolved to a matched, budget-aware product) — with no extra click from the shopper. **The real, more valuable wedge is not "Dell lacks NLP" (they don't) — it's "Dell runs two disconnected AI/search systems that don't share results, facets, or ranking," a systems-integration and consistency gap Algolia NeuralSearch (unified into ONE search+facet+ranking layer) directly solves.** (Evidence: 13-, 13b-, 13c-, 09-, 09b-, 22-, 22b-, 22c-, 22d-.)
...
|---------|----------------------|
| NeuralSearch (consolidation, not net-new capability) | Grid ignores "under 1000" / wrong GPU tier / 400+ undifferentiated SKUs for use-case queries — but Dell's own auto-launched Assistant (same box, same query) already resolves these correctly, live; the wedge is unifying one correct system instead of running two disconnected ones |
